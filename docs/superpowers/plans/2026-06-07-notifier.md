...
**Architecture:** 独立的通知服务模块（notifier）负责判断错误类型、发送通知、管理 feed 禁用状态。统一所有路由的错误处理，使用 `resp.raise_for_status()` 抛出 `HTTPError`。在 refresher 中集成通知逻辑，在 app.py 中检查 feed 禁用状态。

> **实现变更说明**：最终实现将禁用粒度细化为 **feed 级**（按 `cache_key`=`route/feed_id` 区分），而非本计划下文代码示例中的 route 级。即单个 feed 触发业务错误只禁用该 feed，同路由其他 feed 不受影响。对应方法为 `disable_feed` / `is_feed_disabled`，集合为 `disabled_feeds`。下文代码块保留原始设计以反映规划过程，最新接口以 `RSSGen/core/notifier.py` 与设计文档为准。
